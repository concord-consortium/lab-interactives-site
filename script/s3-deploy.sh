...
if [ "$TRAVIS_BRANCH" = "s3cmd-upload" ]; then
  s3cmd sync -c config/s3cmd --quiet --delete-removed --no-preserve --rexclude '^(version/(?!'"$SITE_VERSION"')|branch/)' \
    --exclude '*.html' --exclude '*.js' --exclude '*.css' --exclude '*.json' \
    _site/ s3://interactives-s3.concord.org/ &
  inc='^(?!(version/(?!'"$SITE_VERSION"')|branch/))'
else
  s3cmd sync -c config/s3cmd --quiet --delete-removed --no-preserve --rexclude '^(?!branch/'"$TRAVIS_BRANCH"'/)' \
    --exclude '*.html' --exclude '*.js' --exclude '*.css' --exclude '*.json' \
    _site/ s3://interactives-s3.concord.org/ &
  inc='^branch/'"$TRAVIS_BRANCH"'/'
fi
# Divide sync of gzipped content into multiple processes.
s3cmd sync -c config/s3cmd --quiet --delete-removed --no-preserve --add-header 'Content-Encoding:gzip' \
